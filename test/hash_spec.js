...
describe('hashing Node', function () {
    ...
    it('should return a hash', function (done) {
        ...
        helper.load(hash, flow, function () {
            ...
            n1.receive({ payload: certificate, accessToken: process.env.ACCESS_TOKEN });
        });
    });
    ...
    it('when no certificate is present, a warning will be shown', function (done) {
        ...
        helper.load(hash, flow, function () {
            ...
            n1.warn.should.be.calledWithExactly('Please add a valid JSON certificate to global.certificate or msg.payload');
            done();
        });
    });
});